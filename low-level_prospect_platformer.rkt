...
(define (spawn-player)
  (actor
   ...
   #f
   (list
    (sub (inbound (key-press ?)))
    (sub (inbound (key-release ?))))))

(define left-trie  (move-left))
(define right-trie (move-right))
...
(define ((horizontal-motion-behavior dx) e s)
  ...
  (match e
    [(patch p-added p-removed)
     (define left-added? (not (trie-empty? (trie-project p-added left-trie))))
     (define left-removed? (not (trie-empty? (trie-project p-removed left-trie))))
     (define right-added? (not (trie-empty? (trie-project p-added right-trie))))
     (define right-removed? (not (trie-empty? (trie-project p-removed right-trie))))
     (cond
...
(define (spawn-horizontal-motion dx)
  (actor (horizontal-motion-behavior dx)
         #f
         (list (sub (move-left))
               (sub (move-right))
               (sub (timer-tick)))))
