...
* Platformer Notes
** Should the game logic process be handling (jump) commands?
...
*** a vertical collision, where the player is moving downward (positive dy) results in the player killing the enemy
** Multiple worlds
...
** Rather than having game logic and renderer process maintain a `game-state` struct just have the game logic send its state as a message
